...
export default function Hero() {
  ...
  // Luxury Stagger Container Variant
  ...
  const smoothFadeUp = {
    ...
    show: {
      ...
      transition: {
        ...
        ease: [0.16, 1, 0.3, 1], // Smooth cubic-bezier luxury curve
      },
    },
  };

  return (
    <div className="relative min-h-[85vh] lg:min-h-screen w-full bg-[#050505] text-white font-sans overflow-hidden pt-16 sm:pt-20 lg:pt-24 select-none">
      ...
      {/* ========================================= */}
      {/* BRIDAL IMAGE WITH CINEMATIC ZOOM REVEAL */}
      {/* ========================================= */}
      <motion.div
        initial={{ opacity: 0, scale: 1.08 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.4, ease: [0.16, 1, 0.3, 1] }}
        className="absolute right-0 top-0 w-full lg:w-[60%] h-full z-0 overflow-hidden pointer-events-none"
      >
        <img
          src={bridal}
          alt="The Arti Luxe Luxury Bridal Makeup"
          className="w-full h-full object-cover object-[55%_center] lg:object-[40%_center] opacity-85 lg:opacity-100 filter brightness-[1.02] contrast-[1.04]"
        />

        {/* Soft Mobile Dark Overlay */}
        <div className="absolute inset-0 bg-linear-to-t from-[#050505] via-[#050505]/40 to-transparent lg:hidden" />

        {/* Desktop Left Edge Seamless Fade */}
        <div className="hidden lg:block absolute inset-y-0 left-0 w-2/5 bg-linear-to-r from-[#050505] via-[#050505]/60 to-transparent" />

        {/* Top Vignette (Logo & Navbar Clarity) */}
        <div className="absolute inset-x-0 top-0 h-24 sm:h-28 bg-linear-to-b from-[#050505] via-[#050505]/80 to-transparent z-10" />

        {/* Bottom Vignette */}
        <div className="absolute inset-x-0 bottom-0 h-20 sm:h-24 bg-linear-to-t from-[#050505] via-[#050505]/70 to-transparent" />
      </motion.div>

      {/* ========================================= */}
      {/* SUBTLE AMBIENT GLOW (CLEAN & MINIMAL) */}
      {/* ========================================= */}
      <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
        <div className="absolute -left-25 -top-12.5 h-87.5 sm:h-125 w-87.5 sm:w-125 rounded-full bg-[#D4AF37]/8 blur-[120px]" />
      </div>
        ...
        {socials.map((social, index) => {
          ...
          return (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className={`h-11 w-11 rounded-full border border-[#D4AF37]/60 flex items-center justify-center text-[#D4AF37] transition-all duration-300 bg-black/60 backdrop-blur-md shadow-lg ${social.hoverClass}`}
            >
              <Icon size={18} />
            </motion.a>
          );
      ...
      {/* ========================================= */}
      {/* MAIN HERO CONTENT (STAGGER REVEAL) */}
      {/* ========================================= */}
      <div className="relative z-20 max-w-[1800px] mx-auto min-h-[calc(85vh-64px)] lg:min-h-[calc(100vh-80px)] grid lg:grid-cols-12 items-center px-5 sm:px-8 lg:px-16 pb-8 sm:pb-12">
        ...
        >
          ...
          <motion.div variants={smoothFadeUp} className="flex items-center flex-wrap gap-2 sm:gap-3 mb-2">
          ...
          <motion.div variants={smoothFadeUp} className="mt-5 sm:mt-8 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6">
            <Link
              to="/contact"
              ...
              {socials.map((social, index) => {
                ...
                return (
                  <a
                    ...
                    rel="noopener noreferrer"
                    aria-label={social.name}
                    className="h-10 w-10 rounded-full border border-[#D4AF37]/60 flex items-center justify-center text-[#D4AF37] bg-black/80 backdrop-blur-md shadow-md"
                  >
                    <Icon size={16} />
                  </a>
                );
          ...
          >
            <div className="flex overflow-x-auto sm:grid sm:grid-cols-4 gap-3 sm:gap-6 relative max-w-155 pb-2 sm:pb-0 scrollbar-none">
              {features.map((feature) => (
                <div 
                  key={feature.title} 
                  className="shrink-0 flex flex-col items-center text-center group min-w-18.75 sm:min-w-0"
                >
                  ...
                  <h4 className="mt-1.5 sm:mt-3 text-[8px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.18em] text-gray-100 font-semibold max-w-20 sm:max-w-25 leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
                    {feature.title}
                  </h4>
                </div>
              ))}
